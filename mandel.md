...
float distanceToMandelbrot( in vec4 p )
{
    float di =  1.0;
    vec2 z = p.xy;
    vec2 c = p.zw;
...
{
    // vec2 z0 = ((fragCoord / iResolution.xy) * vec2(4.0, 3.0)) - vec2(2, 1.5);
    // vec2 c = vec2(-0.5125, 0.5213);
    vec2 c = ((fragCoord / iResolution.xy) * vec2(4.0, 3.0)) - vec2(2, 1.5);
    vec2 z0 = vec2(0.0);
    
    vec4 p = vec4(z0, c);

    // distance to Mandelbrot
    float d = distanceToMandelbrot(p);
